...
#define SPI_INTR_ID		    XPAR_FABRIC_AXI_QUAD_SPI_1_INTR
#define INTC_BASE_ADDRESS   XPAR_XINTC_0_BASEADDR
#define XSPI_BASE_ADDRESS   XPAR_XSPI_0_BASEADDR
...
int main(void)
{
    ...
    ConfigPtr =  XSpi_LookupConfig(XSPI_BASE_ADDRESS);    
	if (ConfigPtr == NULL) {
	...
	for (int i=0; i<NUM_SECTORS; i++) {
		...
		xil_printf("Sector %d/%d\r", i, NUM_SECTORS);
	...
	for (int i=0; i<NUM_PAGES; i++){
		...
		if ((i%1024)==0) {
			xil_printf("Page %d/%d\r", i, NUM_PAGES);
...
static int SetupInterruptSystem(XSpi *SpiPtr)
{
    ...
    Status = XIntc_Initialize(&InterruptController, INTC_BASE_ADDRESS);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
